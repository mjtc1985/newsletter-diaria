...
- Set `descartar` to true ONLY for commercial material: changelog entries, "now
  available on X" product notes, customer stories or testimonials, funding rounds,
  and product promotion. The list is closed. Then fill `motivo_descarte` and leave
  `why` and `takeaway` empty.
- Do NOT discard for lack of depth, for being educational, informal or opinion.
  Analysis, tutorials, essays and post mortems stay in even if they look weak: keep
  the summary short and leave `why` empty instead. Discarding is for things trying to
  sell you something, not for things you find shallow.
